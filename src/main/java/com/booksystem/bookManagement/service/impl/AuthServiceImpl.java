...
import com.booksystem.bookManagement.exception.ApplicationException;
...
import com.booksystem.bookManagement.service.AuthService;
import com.booksystem.bookManagement.utils.HelperClass;
import com.booksystem.bookManagement.utils.SecurityConstants;
import com.booksystem.bookManagement.utils.events.publisher.EventPublisher;
...
//    private final SubscriptionEntityRepository subscriptionEntityRepository;
    ...
    public ResponseEntity<ApiResponse<UserSignUpResponse>> registerUser(UserSignUpRequest userSignUpRequest) {
        ...
        boolean isPresent = bookUsersRepository.existsByEmail(userSignUpRequest.getEmail());
        ...
        BookUsers newUser = modelMapper.map(userSignUpRequest, BookUsers.class);
        ...
        BookUsers savedUser = bookUsersRepository.save(newUser);
    ...
    public ResponseEntity<ApiResponse<UserSignUpResponse>>  registerAdmin(UserSignUpRequest userSignUpRequest) {
        ...
        // Maps the UserSignUpRequest dto to a User entity, so it can be saved
        BookUsers newAdmin = modelMapper.map(userSignUpRequest, BookUsers.class);
        ...
        // Save the user to the database
        BookUsers savedAdmin = bookUsersRepository.save(newAdmin);
    ...
    public ResponseEntity<ApiResponse<String>> forgotPassword(String email) {
        if (!bookUsersRepository.existsByEmail(email)) {
    ...
    public ResponseEntity<ApiResponse<String>> resetForgotPassword(ForgotPasswordResetRequest forgotPasswordResetRequest) {
        ...
        Optional<BookUsers> userOptional = bookUsersRepository.findByEmail(email);

        if (userOptional.isPresent()) {
            BookUsers user = userOptional.get();

            user.setPassword(passwordEncoder.encode(forgotPasswordResetRequest.getNewPassword()));

            bookUsersRepository.save(user);
    ...
    public ResponseEntity<ApiResponse<JwtAuthResponse>> login(LoginRequest loginRequest) throws InvalidAlgorithmParameterException, NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, BadPaddingException, InvalidKeyException {
        Optional<BookUsers> userEntityOptional = bookUsersRepository.findByEmail(loginRequest.getEmail());
        ...
        BookUsers userEntity = userEntityOptional.get();
...
//                .subscription(subscription.isPresent() ? subscription.get().getSubscription() : "")
    ...
    public ResponseEntity<ApiResponse<String>> verifyToken(String receivedToken) {
        ...
        Optional<BookUsers> userEntityOptional = bookUsersRepository.findByEmail(email);
        ...
        if (userEntityOptional.isPresent()){
            ...
            bookUsersRepository.save(userEntity);
